...
- (ATVerticalSeparator*) fieldSeparator{
  if (!fieldSeparator_) {
    fieldSeparator_ = [[ATVerticalSeparator alloc] initWithFrame:CGRectMake(0.0, 0.0, 1.0, 1.0)];
  }
...
-(void)layoutSubviewsInRect:(CGRect) layoutRect{
  ...
  CGRect separatorFrame = CGRectMake(frame.origin.x - self.fieldSeparator.frame.size.width -kEdgeSpacerWidth , layoutRect.origin.y , 1.0, layoutRect.size.height);
